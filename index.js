...
// GET /api/records/count — fast count check
...
// GET /api/records/employers — get employer summary (fast, no full data)
app.get('/api/records/employers', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT 
        ...
        COUNT(*)::int as count
      FROM records
      GROUP BY data->>'_e', data->>'_s', data->>'_t'
      ORDER BY count DESC
    `);
    res.json(rows);
  } catch (err) {
...
// GET /api/records — paginated, with optional employer filter
app.get('/api/records', async (req, res) => {
  try {
    const { employer, limit = 5000, offset = 0 } = req.query;
    let query, params;

    if (employer) {
      query = "SELECT data FROM records WHERE data->>'_e' = $1 ORDER BY id LIMIT $2 OFFSET $3";
      params = [employer, parseInt(limit), parseInt(offset)];
    } else {
      query = 'SELECT data FROM records ORDER BY id LIMIT $1 OFFSET $2';
